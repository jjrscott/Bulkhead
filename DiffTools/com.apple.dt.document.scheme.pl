...
sub parseNode
{
	...
	my $content = $$contentRef;
# 	warn $depth, " ", length $content;
	
	testConstant(\$content, $depth+1, "<") || return;
	
	if(testConstant(\$content, $depth+1, "/"))
	{
	    my $name = parseString(\$content, $depth+1) || die;
	    	testConstant(\$content, $depth+1, ">") || die;
	        $$contentRef = $content;
...
#         warn $name;
	...
	while (my $key = parseString(\$content, $depth+1))
	{
		...
		testConstant(\$content, $depth+1, "=") || die Dumper($key, $depth, \%dictionary, substr $content, 0, 200);
		my $value = parseString(\$content, $depth+1, $key);
		die Dumper($key, $value, $depth, \%dictionary, substr $content, 0, 200) if !defined $value;
		$dictionary{$key} = $value;
	}
	...
	unless ($closed) {
	    ...
	    {
	        my $text = $1;
	        push @subnodes, $text if $text =~ /\S/;	    
	    }	
	
		while (my $subnode = parseNode(\$content, $depth+1))
        {
            push @subnodes, $subnode;
        }
	}
...
# 	print Dumper($depth, \%dictionary, substr $content, 0, 200);
	
	$$contentRef = $content;
	
# 	die Dumper(\%dictionary);
	
	return [$name, \%dictionary, \@subnodes];
}
