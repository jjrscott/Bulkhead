...
sub parseArray
{
	...
	while (1)
	{
	    my $value = parseValue(\$content, $depth+1);
...
sub arrayFromFile
{
	...
	$content =~ s!<\?.*?\?>\n?!!g;
	$content =~ s!<plist version="1.0">\n?!!g;
	$content =~ s!</plist>\n?!!g;
	$content =~ s!<\!.*?>\n?!!g;
    $content =~ s!<key>([^<]+)</key>!"$1" = !g;
    $content =~ s!<dict>!{!g;
    $content =~ s!</dict>!}!g;
    $content =~ s!<array>!(!g;
    $content =~ s!</array>!)!g;
    $content =~ s!<(true|false)/>!$1!g;
    $content =~ s!<(string|integer|real)>([^<]+)</\1>!"$2"!g;

#     die $content;


	
# 	my $json = qx(plutil -convert json -o - "$path");
# 	
